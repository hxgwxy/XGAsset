...
namespace XGFramework.LitJson
{
    ...
    public class JsonMapper
    {
            ...
            if (reader.Token == JsonToken.ArrayStart) {
                ...
                while (true) {
                    ...
                    if (t_data.Properties.ContainsKey (property)) {
                        ...
                        if (value_type.GenericTypeArguments.Length >= 1)
                        {
                            var type1 = value_type.GenericTypeArguments[0];
                            var type2 = value_type.GenericTypeArguments[1];
                            if (type1 == typeof(int))
                            {
                                ((IDictionary) instance).Add (Convert.ToInt32(property), ReadValue (type2, reader));
                            }
                            else if (type1 == typeof(long))
                            {
                                ((IDictionary) instance).Add (Convert.ToInt64(property), ReadValue (type2, reader));
                            }
                            else
                            {
        ...
        private static JsonWriter prettyPrintWriter = new JsonWriter() { PrettyPrint = true };
        ...
        {
            prettyPrintWriter.Reset();
            prettyPrintWriter.PrettyPrint = prettyPrint;
            WriteValue (obj, prettyPrintWriter, false, 0);
